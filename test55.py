...
class VerificationSystem:
    ...
    def load_level(self):
        ...
        if self.current_level == 1:  # Original level 3
            ...
            # Get images that weren't used in previous level
    ...
    def display_image_grid(self, rows, cols):
        ...
        for i in range(rows * cols):
            if i < len(self.current_images):
                ...
                is_campus = "campus" in self.current_images[i]
                
                # Load the actual image
                if is_campus:
                    img_path = os.path.join(self.campus_images_path, self.current_images[i])
                else:
                    img_path = os.path.join(self.external_images_path, self.current_images[i])
